...
import {
  publishGitHubDecisionForSlug,
  RELEASE_ROOM_CHECK_NAME,
} from "@/lib/integrations/github-checks";
...
export async function processGitHubWebhook(input: {
  ...
  const eventId = `github:${input.delivery}`;
  ...
  if (
    ...
    payload.check_run?.name === RELEASE_ROOM_CHECK_NAME
  ) {
    summary = `${RELEASE_ROOM_CHECK_NAME} observed`;
    sourceUrl =
      payload.check_run.html_url ?? payload.check_run.details_url ?? prUrl;
  } else if (
    release &&
    (input.event === "check_run" || input.event === "check_suite")
  ) {
    ...
    summary = `${
      ("name" in (check ?? {}) ? payload.check_run?.name : undefined) ??
      "GitHub checks"
    } ${conclusion ?? check?.status ?? "updated"}`;
    sourceUrl =
      check?.html_url ?? payload.check_run?.details_url ?? prUrl;
    ...
    await upsertEvidence(release.slug, {
      ...
      source: "GitHub webhook",
      sourceUrl,
      owner: review?.user?.login ?? sender,
    });
    await addAudit(
      release.slug,
      "evidence_added",
    ...
    summary = `${input.event} did not match an active release`;
  }
