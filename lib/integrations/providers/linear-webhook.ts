  ...
  .object({
    ...
    data: z
      .object({
        ...
        issue: z.object({ identifier: z.string().optional() }).optional(),
      })
      .passthrough(),
  })
  .passthrough();
...
export async function processLinearWebhook(input: {
  ...
  const eventId = `linear:${input.delivery}`;
  ...
  const identifier =
    payload.data.identifier ?? payload.data.issue?.identifier ?? null;
  ...
  if (release && payload.type.toLowerCase() === "issue") {
    ...
    await upsertEvidence(release.slug, {
      ...
      description: hasCriteria
        ? `Acceptance criteria found on ${identifier}.`
        : `No explicit acceptance criteria found on ${identifier ?? "the linked issue"}.`,
      status: hasCriteria ? "passed" : "failed",
      required: true,
      source: "Linear webhook",
      sourceUrl: payload.url ?? release.linearUrl,
      owner: "Product",
    });
    await addAudit(
      release.slug,
    ...
    summary = `${payload.type} ${payload.action} did not match an active release`;
  }
